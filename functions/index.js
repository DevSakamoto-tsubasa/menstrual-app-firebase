// functions/index.js - 修正版完全版 (関数定義順序修正)
 ...
 * Web UI リンク生成 (修正版)
 * @param {string} userId - LINE ユーザーID
 * @param {string} page - ページ名 (setup, dashboard, calendar, settings)
...
設定完了後、LINEで「開始日を入力」から記録を始められます！
...
※リンクは24時間有効です`;
  ...
  if (msg === '設定ページ' || msg === '設定画面' || msg === '設定') {
    const settingsLink = generateWebUILink(userId, 'settings');
    return `⚙️ 設定ページ
